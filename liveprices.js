...
// begin setup CLI
//
 const price_update = {"pairs":["XAU/USD","XAG/USD"]}

var setupCLI = () => {
	if(typeof(price_update.pairs) === 'undefined') {
		console.log('command error: "pairs" parameter is missing.');
	} else {
		subscribe(price_update.pairs);
	}
	//exports.setupCLI = {'result' : setupCLI};
/*
...
var priceUpdate = (update) => {
	try {
		...
		// Be aware that .toFixed returns a String
			...
			var result = element.toFixed(5);
			
			return result;			

			// first try to export data fails
			/*
			exports.result = {
				'result' : result
			   };*/ // ends here
		...
		// correct way to send data to main.js file
		...
		//console.log(`@${jsonData.Updated} Price update of TEST [${jsonData.Symbol}]: ${jsonData.Rates}`);	
	} catch (e) {
